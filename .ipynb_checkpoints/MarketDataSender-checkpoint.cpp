...
#define DEFAULT_INPUT "msft_orders.bin"
...
int main() {
  ...
  std::ifstream file(DEFAULT_INPUT, std::ios::binary);
  if (!file.is_open()) {
    std::cerr << "ERROR: Cannot open " << DEFAULT_INPUT << std::endl;
    return 1;
  }

  std::cout << ">>> SENDER ONLINE (" << DEFAULT_INPUT << " -> Port " << PORT
            << ") <<<" << std::endl;
