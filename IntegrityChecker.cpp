...
#define DEFAULT_TRUTH "msft_truth.bin"

int main() {
  // Load truth data
  std::ifstream file(DEFAULT_TRUTH, std::ios::binary);
  if (!file.is_open()) {
    std::cerr << "ERROR: Cannot open " << DEFAULT_TRUTH << std::endl;
    return 1;
  }
  ...
  std::cout << "Loaded " << truthLog.size() << " truth snapshots" << std::endl;
