...
int main() {
  ...
  for (auto ticker : tickers) {
    ...
    for (auto date : dates) {
      ...
      std::string orders_file = "../data/converted/daily/" + std::string(ticker) + 
                                "_" + std::string(date) + "_orders.bin";
      std::string truth_file = "../data/converted/daily/" + std::string(ticker) + 
  ...
  std::cout << "Failed (<95%):     " << (total_days - passed_days - skipped_days) << "\n";
  
  double success_rate = (double)passed_days / (total_days - skipped_days) * 100.0;
