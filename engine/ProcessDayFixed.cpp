...
int main(int argc, char *argv[]) {
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " <orders.bin> <truth.bin> <output.bin>\n";
    return 1;
  }
  ...
  double acc =
      (matched + failed > 0) ? (100.0 * matched / (matched + failed)) : 0.0;

  // Silent unless error
  if (acc < 95.0) {
    std::cerr << "WARNING: " << acc << "% accuracy (skipped "
              << skipped_premarket << " pre-market orders)\n";
  }
